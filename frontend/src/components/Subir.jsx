  ...
  try {
    ...
    if (response.ok) {
      ...
      setSnackbar({
        open: true,
        message:
          'Error: ' + (data?.message || 'No se pudo obtener sugerencias'),
        severity: 'error',
      })
      return null
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSnackbar({
          ...
          message:
            '❌ Error: ' +
            (data?.errors?.image || 'No se pudo procesar la solicitud.'),
          severity: 'error',
        })
      }
    } catch (error) {
      ...
      <div className="subir-form-center">
        <form className="subir-form" onSubmit={handleSubmit}>
          ...
          {showSuggestionsSection ? (
            <div
              className={`description-suggestions-row ${showSuggestionsSection ? 'two-columns' : 'one-column'}`}
            >
              {/* Columna: Descripción */}
