...
class RatingListCreateView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(
                response=RatingSerializer, description="Valoración creada correctamente"
            ),
            400: {
                "type": "object",
                "properties": {
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                if post_id is None:
                    return Response(
                        {"success": False, "message": "El campo 'post' es requerido."},
                ...
                return Response(
                    {"success": False, "message": "La publicación no existe."},
            ...
            try:
                ...
                if not post.allows_ratings:
                    return Response(
                        {
                            "success": False,
                            "message": "Esta publicación no permite valoraciones.",
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                if Rating.objects.filter(post=post, rater=user).exists():
                    return Response(
                        {
                            "success": False,
                            "message": "Ya has valorado esta publicación.",
                        },
                ...
                logger.error(f"Error al crear la valoración: {str(e)}")
                return Response(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not post_id:
            return Response(
                {"success": False, "message": "El parámetro 'post_id' es requerido."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            if post.author == user:
                return Response(
                    {
                        "success": False,
                        "message": "No puedes valorar tu propia publicacipon",
                    },
            ...
            logger.info(
                f"Valoración obtenida: Rater {rating.rater} - Post ID {rating.post} - Valoración {serializer.data}"
            )
        ...
        # Si el Rating no existe se devuelve success: false
            ...
            return Response(
                {
                    "success": False,
                    "message": "El usuario no ha valorado esta publicación",
                },
                status=status.HTTP_200_OK,
            )

        except Post.DoesNotExist:
            logger.warning("No se puede obtener la valoración de un post inexistente.")
            return Response(
        ...
        try:
            ...
            if rating.rater != request.user:
                return Response(
                    {
                        "success": False,
                        "message": "No puedes actualizar esta valoración.",
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            if not rating.can_be_edited():
                return Response(
                    {
                        "success": False,
                        "message": "Solo puede actualizar la valoración durante las primeras 24 horas.",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if serializer.is_valid():
                ...
                logger.info(
                    f"Valoración actualizada: Valorador {rating.rater} - Post ID: {rating.post.id} - Campos modificados: {list(request.data.keys())}"
                )
                ...
                logger.warning(
                    f"Error de validación al actualizar la valoración ID {pk}: {serializer.errors}"
                )
                return Response(
                    {
                        "success": False,
            ...
            logger.warning("No se puede actualizar una valoración inexistente")
            return Response(
                {"success": False, "message": "Valoración no encontrada."},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class PostRatingsView(APIView):
    ...
    def get(self, request, post_id):
        ...
        try:
            ...
            logger.warning("No se pueden obtener el promedio de un post inexistente.")
            return Response(
                {"success": False, "message": "Post no encontrado."},
                status=status.HTTP_404_NOT_FOUND,
            )
